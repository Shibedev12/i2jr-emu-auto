...
function setAlertPage() {
  // Check if alerts exist first
  if (alerts.length == 0) {
    return; // Skip alert page setup if no alerts
  }
  
  try {
    // Single Alert page
    const singleAlertText = getElement('single-alert-text');
    if (alerts[0] && singleAlertText) {
      singleAlertText.innerHTML = alerts[0];
    }
    
    // Multiple Alert page
    if (alerts.length > 1) {
      const alertsContainer = getElement('multiple-alerts-container');
      if (alertsContainer) {
        let altxt = "";
        for (var i = 0; i < alerts.length; i++) {
          altxt += "<div class='multiple-alert-item'>" + alerts[i] + "</div>";
        }
        alertsContainer.innerHTML = altxt;
      }
    }
  } catch (e) {
    console.error("Error setting alert page:", e);
    // Continue execution even if there's an error with alerts
  }
}
